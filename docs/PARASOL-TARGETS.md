...
> compiler I run in Parasol (`parasol cbasic.psol myprogram.bas -> a.out`) or
...
## Parasol does not run programs

`bin/parasol` is a translator. A `.psol` goes in and Solveig source comes out,
and that is all it does — so `parasol cbasic.psol myprogram.bas` is not a
sequence that could exist. **`cbasic.psol` is not a compiler Parasol runs. It is
source that becomes a compiler.**

The second sequence is the right one, with one step in front of it:

```sh
parasol cbasic.psol -o cbasic.sol      # Parasol  -> Solveig source
...
**And it needs nothing from Parasol at all.** Parasol is only the language the
compiler was written in.

## So there are two senses of "target", and only one is a Parasol question

| | |
| --- | --- |
| **What Parasol's own back end emits** | Solveig source. A Parasol question, answered in the README under *What Parasol is allowed to know about Solveig*. |
| **What a program written in Parasol emits** | Whatever that program writes. Not a Parasol question, any more than what a C program writes is a question about C. |

The BASIC compiler is the second. It is a program, and Parasol's involvement
ends when the program is compiled.

## Could Parasol itself emit machine code?
...
-agnostic; `parasol/src/emit.c` is the only file that has ever heard of Solveig.
...
[does-it-pay.md](PARASOL-DOES-IT-PAY.md) is what seven programs and four readers who had
...
```sh
parasol cbasic.psol -o cbasic.sol && solas cbasic.sol -o cbasic.sob
...
end became a declared thing the way the grammar already is, and "Parasol targets
...
it. `parasol/include/parasol/lex.h` says so in a sentence now, instead of a
directive standing empty in every file. See COMPLETED.md 14.

## Why a code generator is the right first real program

A BASIC compiler written in Parasol is a better test of the whole idea than
