...
is left-recursive in every grammar that spells it out, and Parasol reads it from
...
about what may follow an `expr` fragment exist for the reason Parasol bans two
...
**A word after a hole works because Parasol's expression grammar cannot consume
...
Rust's follow-set rules are stricter than Parasol's for exactly this reason:
they are future-proofing against their own grammar growing. Parasol has taken
...
And it is the better customer for Parasol than another arithmetic example: a
grammar notation is precisely what a pattern language should be good at, and a
toolkit whose own notation is a Parasol dialect is this project's premise
pointed at itself.
...
### A Prolog dialect written in Parasol

Yes, and **Parasol helps with less of it than it looks.** `ideas.md` has already
...
Parasol changes none of that. What it changes is the notation, and the notation
...
rule. This is a question about Parasol, and the answer is the interesting one.

**A predicate has to run while compiling, and Parasol has no evaluator.** That
is deliberate rather than missing: Parasol emits Solveig source and lets Solveig
...
| | |
| --- | --- |
| Write an evaluator | Parasol gains a second language, and the question of whether it is the same language as the object language — the tower — has to be answered rather than avoided. |
...
inside Parasol, and whether the hosting Solveig already tried for HTTP is the
...
Parasol is the third. And the surface is declared rather than inferred:
...
**It costs the build.** `bin/parasol` would link `libsol.a`, and *the build needs
...
`extend.h`; Parasol with guards is C calling into Solveig through `embed.h`. Two
...
no tool can read a `.psol` without running it. That is the line this whole design
...
Once `embed.h` is in, a guard may be written in *Parasol*: compiled to Solveig
...
5. **A parser toolkit written in Parasol.** Not a feature. The best test of
