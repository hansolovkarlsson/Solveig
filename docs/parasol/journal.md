...
*Frozen on 2026-09-15: Parasol's journal from its first day to the day it
became a member of Solveig's toolkit. From that day a day of work on Parasol
is a day of work on Solveig and is in [Solveig's journal](../journal.md).
Nothing is appended below this note.*

*What a day of work on Parasol actually consisted of, newest first.*
...
for Solveig, `@with rect { x := #0. ... }`, that reads every statement as a
slot write on one receiver; it was scoped and held there, in Solveig's
`ideas.md`. Then he asked whether Parasol could carry it instead.

### The block form cannot be a template, and the line form found the recorded limit

A `@syntax with <o> <body: block>` places the block whole; nothing walks the
statements inside a hole, and repetition has been declined three times on the
roadmap. The line form was tried, `on <o> => it := o` and
`slot <n: name> is <e> => it:n := e`, and emitted `it:n := #0` for every
slot: `instantiate` replaces a parameter only where it stands as a name node,
and a selector is text on the send node. That is `programs/grammar`'s
prediction 4, *a template cannot build a selector out of a hole*, met a
second time. The roadmap has the entry, and it says why the second customer
does not get what it came for even if the feature were built: the output
would say `it:x`, and the greppable `.sol` that made this the attractive home
is lost either way. Count stays at one.

### The name

Hans said he was not totally sold on *Proto* and offered `pretex`. The
answer was no, on *pretext*, on PreTeXt, and on *preprocessor* being the
design this compiler avoids. Three rounds of names followed, none taken,
until he offered *Parasol*, which was better than it first sounded: plain
English, `sol` in the tail, *para-* as *beside* and *paraphrase* as what the
output is. [COMPLETED.md](COMPLETED.md) 18 has the table. The extension
went to `.psol` in the same breath, since `.pro` had been half the
objection.

### What the rename was checked against, and what it left alone

The same shape as 2026-09-01: a survey first, 90 tracked files under the
directory and 24 `.pro`, then `git mv` for the directory, the C source
directory, the headers, one document and every module, then a respelling
over the tree in four rules, then the residue read line by line. Two things
the rules could not decide were done by hand. The `.sol.map` header and the
editor's scope names moved with the name; the object model's `proto`, in
Solveig's own sources and documents, did not, and the rules were written to
miss it. **And the records of the first rename were put back** after the
respelling had reached them, because they had started to say that `.psol` is
four characters and that `Pro` keeps a three-letter abbreviation: a record
of a choice keeps the words of the choice.
...
untested in `examples/vectors.psol`: a second file may declare `+` to mean
...
oracle. The sharpest was the second: that `lib/arith.psol` would serve the
...
`limbs.psol` came out at five lines: the base, its width, `digit(t)` and
`carry(t)`, over `lib/control.psol` unchanged. The library is 126 lines, the
...
back: 27 to `bignum.psol:55`, 16 to `calc.psol:51`. Prediction 5 held and Parasol
...
was built for. Two fixes, neither built: Parasol keeping a line break inside an
...
**Parasol lives in Solveig's tree now**, as `parasol/`, with this history intact
...
Parasol went beside Solveig rather than inside it, argued from something stronger
...
was the other half of the same fact: Parasol emits Solveig and nothing else, so
every change to the substrate is a change Parasol has to follow, and two
...
this speaks to by construction, and `PARASOL_SOLVEIG_MINIMUM` stays in
...
to `..`. The parent's `make` builds `bin/parasol`, its `make test` runs this
suite after its own, and its `make install` installs `parasol` and `lib/*.psol`.

The records stay here as Parasol's own: this journal, [CHANGELOG.md](CHANGELOG.md)
...
does not refuse for this directory. The version number stays Parasol's until
there is a reason to fold it in. The standup is the parent's. The repository at
github.com/hansolovkarlsson/Proto is archived with a pointer to `Solveig/parasol`.
...
the defect tested, one exit from `parasol_source_read`. Suite **63, 6, 69 and
...
commit did what [conventions.md](../method.md#what-came-in-from-parasols-conventionsmd-and-what-was-retired) says to do: it grepped for the
...
`examples/clike.psol` braces every body, the reader took their syntax *by direct
...
to hand a reader `lib/clike.psol` with its warning removed, and that was refused
...
it — `examples/forms.psol` declares four forms and types no hole at all.

**They typed `<b: block>`, copying `lib/control.psol`'s `repeat`, and then braced
every use anyway** — the model this time being `examples/dialect.psol`'s
`repeat #3 times { "tick":display }`, a one-statement body wearing braces. Run
3's finding one level up.

### Four runs, and no reader has ever seen a Parasol diagnostic
...
`parasol` exited 0, `solas` exited 0, and `solvm` reported `undefined name
...
> Solveig's diagnostics at every point where Parasol's own would have had to work.
...
[second-reader.md](../PARASOL-SECOND-READER.md)'s run 2 put the one-line `REFERENCE.md`
...
page**. That was recorded in [second-reader.md](../PARASOL-SECOND-READER.md), again in
[does-it-pay.md](../PARASOL-DOES-IT-PAY.md), and a third time in yesterday's journal entry
...
checking them.** `examples/clike.psol` was called the shortest complete example
and is not — `dialect.psol` is 41 lines to its 45. And `lib/clike.psol` was
...
cascade; run 2's reader was warned off by `lib/clike.psol`'s eleven lines and
said so unprompted.
...
`lib/clike.psol`'s author paid it. The next dialect with a `block` hole has an
...
> A `@use` path is not normalised. `examples/../lib/control.psol` is what a
...
A `ParasolSource` carries `identity` beside `path` now — `realpath`, falling back
to a copy where there is nothing on disk to resolve. **Display is unchanged and
deliberately so**: the cycle error still names `./././a.psol`, because that is
...
struct did. `parasol_source_read` has one exit now, which is the argument against
remembering.
...
not written into [conventions.md](../method.md#what-came-in-from-parasols-conventionsmd-and-what-was-retired) yet because one day is an
...
*`lib/arith.psol` has no `<=`* and *one is still not enough*. Both were overtaken
...
[conventions.md](../method.md#what-came-in-from-parasols-conventionsmd-and-what-was-retired) says so now.
...
### Then Solveig's README began linking Parasol
...
dependencies beyond a C11 compiler and `make`* stays true, and **Parasol lives
...
[does-it-pay.md](../PARASOL-DOES-IT-PAY.md) has ended on the same sentence since it was
...
`examples/clike.psol` has neither of.
...
It has. `lib/clike.psol` spends eleven lines at the `else` form's own
...
[conventions.md](../method.md#what-came-in-from-parasols-conventionsmd-and-what-was-retired) gained the sentence **write the narrative to
...
**Two questions wearing one sentence**, which is what [targets.md](../PARASOL-TARGETS.md)
...
- A BASIC **interpreter written in Parasol** — yes, and `programs/ember` was
  already 90% of the shape.
- **BASIC as a Parasol dialect** — partly, and compiled rather than interpreted.
...
`lib/arith.psol` shipped `<`, `>` and `==` alone from the first commit. The entry
...
- And one that is not a count at all: `REFERENCE.md` described `lib/clike.psol`'s
...
`parasol/` was touched. One program, one shipped dialect completed, one roadmap
...
> Parasol runs the rules on the code and replaces the parts that matches?
...
Pascal compiler written in Parasol still cannot declare `|`, and a Parasol that
emitted Pascal still could not either, because the ambiguity is in `{ a | b }`
and that is Parasol's own syntax.
...
was. Both are now in [what-is-parasol.md](../PARASOL.md), kept as the questions
they were rather than rewritten into statements, and the pipeline is drawn four
times in [pipeline.html](../PARASOL-PIPELINE.html) — `3156d26`.
...
on a build this project has already declined** — [targets.md](../PARASOL-TARGETS.md)
...
Fifteen lines, touching no `.psol`, and the exact code a selector would need
...
`\/` had been doing two jobs — logical *or* in `lib/arith.psol`, bitwise *or* in
`examples/utf8.psol` and `programs/digest/sha2.psol` — and `~` two as well,
...
`lib/clike.psol` already declares it prefix-not.
...
`programs/digest/README.md` says composing `sha2.psol` with `lib/control.psol`
collides on four operators. Before repeating that in another document, it was
run. `parasol` segfaulted.
...
**And the claim was wrong anyway.** Parasol reported nine redeclarations, not
...
and a standing agreement in [conventions.md](../method.md#what-came-in-from-parasols-conventionsmd-and-what-was-retired), and the suite is
...
`ledger` found that `#-1225` is an error in Parasol and a valid integer in
...
It had not. Comparing every form in Solveig's grammar against Parasol, one file
...
Parasol refusing something Solveig takes — a smaller language and an honest error.
That one was Parasol *accepting* something Solveig refuses, and emitting it, so a
`.psol` compiled clean and produced a `.sol` `solas` rejected. **Parasol emitting
...
Parasol cannot copy because a dialect may declare `=` anywhere — `lib/clike.psol`
...
number outside a `@expr` region and treats it as the operator inside one. Parasol
...
Asked whether `lib/arith.psol` should be completed with the bitwise operators:
**no**, and the tree makes the case rather than taste. Bitwise has one usable
customer rather than two — `sha2.psol` cannot share a file, its `<<` being masked
...
`greaterOrEqual`, and a `not` that `arith.psol` had been spelling plainly all
along while `clike.psol` wrapped it. `digest` runs 920 instructions fewer, and
what is left in `clike.psol` is three templates, each one a message cannot be.
...
[targets.md](../PARASOL-TARGETS.md) has said since 0.1.0 that the only thing this project
...
[does-it-pay.md](../PARASOL-DOES-IT-PAY.md) is the four weighed together, with the numbers
...
Solveig writes twenty-three `bitAnd`s by hand and Parasol writes none. Solveig's
file has 24 in code, of which **18 are `bitAnd(mask)`**; Parasol's has **0 masks
...
but the dialect borrowed that from Solveig rather than inventing it. `sha2.psol`
...
It also reversed an answer given the same morning. `lib/arith.psol` has no `<=`,
...
The lexer was never touched. `|` is still `PARASOL_TOK_BAR` and still not an
operator character, so a tool can tokenise any `.psol` knowing nothing about its
...
So **`lib/clike.psol` has had a logical xor since it declared `!=`**, and nobody
...
[REFERENCE.md](../PARASOL-REFERENCE.md), the page you look a spelling up in;
[does-it-pay.md](../PARASOL-DOES-IT-PAY.md), the answer to the only question the project
...
a defect in Parasol. **13** is the stale version count: a number correct on the day
...
that were on the table, and why full-word identifiers with a short `.psol`
...
The folder is `~/Projects/Parasol` now, moved at the end of the day rather than
...
`PARASOL_TOK_OPERATOR` is spelling only and `@infix` is meaning, so the real question
...
nowhere. That is 0.9.0, and `lib/clike.psol` stopped apologising for `\/`.
...
notation at all.* Parasol's ROADMAP has claimed the answer to that since 0.1.0
...
collisions against `lib/control.psol`, reported exactly as designed, and the
...
[targets.md](../PARASOL-TARGETS.md), [rules-and-logic.md](../PARASOL-RULES-AND-LOGIC.md),
[solveig-notes.md](../PARASOL-SOLVEIG-NOTES.md), and this document set. 5,187 lines of C11,
...
The second decision was where Parasol lives. Solveig existed already, and
solveig-sdl's Makefile had written the rule down about itself — *this is an
extension, so it is not part of Solveig* — so Parasol went beside it rather than
...
is single-pass with no tree, so Parasol owns one, and that settles what Parasol
...
emitter, as [targets.md](../PARASOL-TARGETS.md) argues — or stop existing. That is a
...
without running it. [rules-and-logic.md](../PARASOL-RULES-AND-LOGIC.md) carries the whole
argument.
