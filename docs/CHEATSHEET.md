...
| [recordset.sol](../lib/recordset.sol) | `recordset:on(table)`; `first` `next` `previous` `last` `at`; `current` `position` `count`; `addNew` `save` `revert` `delete` `changed`; `find` `all` `orderBy` | one record at a time, between a table and a form |
