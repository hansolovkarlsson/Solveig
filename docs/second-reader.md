...
| **4. The `else if` chain misdirects — and the prediction is about recovery, not about the message.** | Checked before this was written, so the message is not what is being predicted: `if (a) { … } else if (b) { … }` reports `lib/clike.pro:72:43`, points into a template the reader did not write, and **names their own file nowhere but the summary line.** That is [POSTMORTEM.md](POSTMORTEM.md) 22. What is open is whether a reader gets back. Predicted: **they do not** — they find the nested-brace workaround by reading the comment in `examples/clike.pro`, not by following the error. If so it is the first test of the map-and-diagnostics guarantee by somebody who could not already know the answer, which is the only kind of test it has. |
