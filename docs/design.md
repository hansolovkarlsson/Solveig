...
That first shape was a *discovery* rather than a decision. The twenty-three<!--count programs-->
programs in
...
this language*, which is a true sentence about twenty-three<!--count programs--> programs
and an empty one
...
| **Graphics** | Nothing structural: [GTK4 and SDL2](extensions.md) both draw, and a canvas re-tested the mechanism on 2026-08-30 without needing anything new, and **the controls a form is made of arrived in the GTK binding on 2026-09-17**, seven widgets and eighteen messages, with a form over a table written in Solveig above them ([the entry](ideas.md#forms-over-a-database-the-way-vb3-did-it-scoped-2026-09-17)). At scale it meets the value representation, below. |
| **Stateful work** | Reflection cannot write — no `slotAtPut`, no `clone`, no re-parenting, and no reading a global by computed name ([2.14](ROADMAP.md#214-loose-ends-from-the-decided-items)). **Narrowed on 2026-09-15**: `object:new(dictionary)` makes slots from run-time names at construction, asked for by a [database row](ideas.md#the-database-as-objects-and-the-first-slot-made-from-a-run-time-name-scoped-2026-09-15); everything after construction stands as written. |
| **Databases** | Was blocked outright until [3.22](COMPLETED.md#322-a-file-is-read-whole-or-not-at-all--done) closed on 2026-08-31; a ranged read exists now. Still absent: a **positioned write**, a file identity ([6.39](COMPLETED.md#639-a-program-cannot-tell-whether-two-paths-are-the-same-file--done)), and anything about locking or flushing. **Reached on 2026-09-15** by [sqlite](programs.md#sql-and-sqlite-an-sql-shell-and-the-library-that-reads-and-writes-the-file), which reads and writes SQLite's own file format and is held against `sqlite3`: the write was wanted at the step the plan named and was built the same day as [3.27](COMPLETED.md#327-a-file-is-written-whole-or-appended-to-and-nothing-in-between--done), `writeFile(path, from, text)`, with the table before and after; locking and flushing were not wanted by a single process checked after it exits. **A form over it on 2026-09-17**: `lib/recordset.sol`, one record at a time between a table and a window, and the window in the GTK binding; what it wanted from the language was a column written by a name held in a value, answered by `set` making the record again, and not the `slotAtPut` the *Stateful work* row above still lacks. |
