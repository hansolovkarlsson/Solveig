...
| **Databases** | Was blocked outright until [3.22](COMPLETED.md#322-a-file-is-read-whole-or-not-at-all--done) closed on 2026-08-31; a ranged read exists now. Still absent: a **positioned write**, a file identity ([6.39](COMPLETED.md#639-a-program-cannot-tell-whether-two-paths-are-the-same-file--done)), and anything about locking or flushing. **Reached on 2026-09-15** by [sqlite](programs.md#sql-and-sqlite-an-sql-shell-and-the-library-that-reads-and-writes-the-file), which reads and writes SQLite's own file format and is held against `sqlite3`: the write was wanted at the step the plan named and was built the same day as [3.27](COMPLETED.md#327-a-file-is-written-whole-or-appended-to-and-nothing-in-between--done), `writeFile(path, from, text)`, with the table before and after; locking and flushing were not wanted by a single process checked after it exits. |
