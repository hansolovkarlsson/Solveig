...
run the binaries as a shell would — **756<!--count claims--> claims on every build**, in about
...
two documents a newcomer actually reads. 340<!--count docs-claims--> claims
...
```text
[expect.sol](../programs/expect.sol) checks 756<!--count claims--> claims
```
