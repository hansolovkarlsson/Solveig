...
run the binaries as a shell would — **757<!--count claims--> claims on every build**, in about
...
two documents a newcomer actually reads. 341<!--count docs-claims--> claims
...
```text
[expect.sol](../programs/expect.sol) checks 757<!--count claims--> claims
```
