...
run the binaries as a shell would — **759<!--count claims--> claims on every build**, in about
...
two documents a newcomer actually reads. 343<!--count docs-claims--> claims
...
```text
[expect.sol](../programs/expect.sol) checks 759<!--count claims--> claims
```
