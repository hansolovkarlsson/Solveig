...
**What Parasol finds in Solveig goes in [solveig-notes.md](solveig-notes.md).**
...
**Solveig's README links Parasol, since 2026-09-03.** The hold was set on
2026-08-31, when Parasol was a front end with no expander and pointing Solveig's
...
dependencies beyond a C11 compiler and `make`* stays true. **Parasol is outside so
...
— for a language that publishes a **library** Solum consumes, where Parasol emits
...
**The name was Proto from 2026-09-01 to 2026-09-14, and is Parasol since.**
The second rename was a choice rather than a move, and the names put and not
taken are in [COMPLETED.md](COMPLETED.md) 18. The same consequence holds one
step on: a surviving `Proto`, `proto_`, `PROTO_` or `.pro` in this tree is a
defect, **except in the records of the first rename**, which keep the name
they chose because they say what was chosen then. The archived repository
`hansolovkarlsson/Proto` keeps its name, as the Phoenix one did.
...
made a branch and left uncommitted changes to `parasol/src/reader.c` in this
...
```sh
git worktree add ../Parasol-<name> -b <name>
```
