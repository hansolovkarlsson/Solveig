...
## 1. A tree of Parasol's own — done, 0.1.0
...
Solveig to suit Parasol, and single-pass is the right shape for a compiler with
fixed syntax. Own one.

**Why this shape.** Owning a tree is what makes Parasol a compiler rather than a
preprocessor, and it settles what Parasol *is*: not a bolt-on to Solas, but a
...
`lib/clike.psol` declared C's *or* as `\/` and wrote a paragraph apologising for
...
run, but the practical one: **today any tool can tokenise any `.psol` without
...
though it were true of everything. **A rule decides it**, and Parasol had the
...
**any tool can tokenise any `.psol` without knowing what a dialect is** — is
...
Neither is right. What stands in the way of `|` is **Parasol's own block syntax**,
...
already existed — `PARASOL_TOK_OPERATOR` is spelling only and `@infix` is meaning —
...
a name and a span in `ParasolDialect`, and nothing in the tree ever read either —
...
touches no `.psol`, and it is the code a selector would need later. The argument
...
header.** `examples/forms.psol` said `@language solveig.` and then declared `+`,
...
Solveig* — is the same for every `.psol` there will ever be and is already
...
right one in its own sentence — *"Parasol targets ARM64" is a line in a file* — so
if a second emitter is ever built, what gets added is `@target`, naming what it
selects.

**What it cost.** Nine `.psol` files, ten fixtures in `tests/test_use.c`, the
...
| | |
| --- | --- |
| **Free** — `#-45`, `$FF08`, `1e10` | Nothing in Parasol objected. Nothing else may begin with `#`, `$` was not a token at all, and a float simply stopped at its fraction. |
| **A defect, not a gap** — `"\q"` | The only difference pointing the *other* way. Parasol took any character after a backslash; Solveig has five escapes. So a `.psol` compiled clean and emitted a `.sol` `solas` refused — **Parasol emitting invalid Solveig**, which is the single failure the map and the run-every-example discipline exist to prevent, and which neither caught because no example has a bad escape. |
| **A parse rule** — `#[a = b]` | Called free on the strength of the lexer and was not. Solveig writes `pair = sum "=" expression` and settles the ambiguity by *level*; Parasol cannot copy that, a dialect being free to declare `=` anywhere and `lib/clike.psol` putting it at 10. |
...
declared one. It is a *parser* rule, so nothing about tokenising a `.psol`
...
`@expr` region and treats it as the operator inside one. Parasol can have neither
...
**What it did not cost.** Any part of *a tool can tokenise a `.psol` without
...
**The problem.** `README.md`, `parasol/src/lex.c` and entry 12 above all said a
...
That is the second place in Parasol where a context outranks a declaration, and
...
`PARASOL_TOK_BAR` and still not an operator character, so **a tool can tokenise
any `.psol` knowing nothing about its dialect**, which is the property entry 12
...
`lib/clike.psol` spends at its own `else` declaration.
...
**Once *per dialect*.** `lib/clike.psol`'s author paid it, and every future
...
| **Say it in `lib/clike.psol` and leave the compiler alone** | Already done, and it worked — run 2's reader never saw the error. It works for **this** dialect, written by somebody who had already paid. It is the thing being generalised, not an alternative to it. |
...
## 18. The name, a second time — done, no version, 2026-09-14

*Not a roadmap item, as 13 was not. It is here because the names were put
with reasons before one was taken, which is what this file keeps.*

**The problem.** Hans was not sold on *Proto*, and the unease had a shape once
looked for: `.proto` and `protoc` belong to Protocol Buffers, `.pro` to qmake
and to Prolog, and in a prototype-based language the word points at the
object model, which is the one thing this compiler is not about. None of it
was fatal on 2026-09-01, when the name had to leave Phoenix in a morning and
`.pro` was chosen over `.proto` for exactly the first collision. Thirteen days
and seven programs later there was time to choose rather than to move.

**The options, in the order they were put.** Hans's first was `pretex`, and
the answer was no on three counts: it reads as *pretext*, an excuse; PreTeXt
is an existing authoring language; and it says *preprocessor*, which is the
design this compiler was built to avoid, there being no rewrite-and-rescan
over text. Then three rounds:

| | | |
| --- | --- | --- |
| **Solfa** | tonic sol-fa is *a notation* invented so people could read music without the staff, and `Sol` is in it as the note | not exciting |
| **Rubric**, **Målform**, **Idiom**, **Fortale** | a red heading that says how to read what follows; Norwegian for a written standard of the language; a dialect's turn of phrase; Norwegian for *preface* | the grading meaning, the ring and *malformed*, generic, a pun in English |
| **Kenning** | Old Norse for a made-up compact expression the poem defines by use, which is what a form is; *ken*, to know | not sold |
| **Argot**, **Tiro**, **Skald**, **Dialect** | a jargon a group makes for itself; Cicero's scribe and the first shorthand; the poet who makes the kennings; what the documents already call a `.psol` file | French; *novice*; names the maker; generic |
| **Parasol** | Hans's second suggestion. Plain English; `sol` in the tail as Solas and Solis have it in the head; `para-` is *beside*, and *paraphrase* is what the output is; the literal object is a layer held between the reader and the sun, which is what a dialect is | the layer also shades what is under it, which the documents already say of a dialect |
| **Presol**, **Solo** | *before Solveig*; *a grammar holds for that file and no other* | coined; says standalone, and taken by small tools |

**Parasol, taken.** The one word that says what the thing is and joins the
family in the same breath, and free as a command name.

**The extension, which was half the objection.** `.pro` could have stayed,
and would have kept the qmake and Prolog collision. `.parasol` is
unambiguous and long. **`.psol`** reads as *para-sol* at a glance, is
five characters, and collides with nothing; taken. It is one character
longer than `.pro`, which is the whole of the code change: 13's *luck* that
`.phx` and `.pro` were the same length is spent, and `default_output_path`
counts five.

**What it cost.** Nothing in the language. 2496 lines out and 2496 back in,
the same shape 13 reported; the suite here and Solveig's green before and
...
| **`<=`, `>=` and `!=` in `lib/arith.psol`** | **Taken**, on 2026-09-02, and the first entry a customer settled *for* rather than against. `programs/prose` wanted `<=` and wrote around it; `programs/basic` wanted `<=` and `>=` five times and `!=` four. Both were rewritten to use them the day they landed, which is the check that the customer was real. |
