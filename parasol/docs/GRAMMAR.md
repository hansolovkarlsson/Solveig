...
Parasol file there will ever be.
...
place in Parasol where a context outranks a declaration.** `#[k = v]` reads as a
...
lives; Parasol cannot copy that, because a dialect may declare `=` at any
precedence and `lib/clike.psol` puts it at 10.
...
file, in a block and in a group alike. That is Solveig's rule and Parasol does
...
treats it as the operator inside one, and Parasol can have neither half — it has
...
A `.psol` holding directives and nothing else, reached with `@use "name.psol".`
...
is no third thing for a `.psol` to be, and a dialect that wants both ships a
`.sol` beside itself.

**Looked for beside the file using it, then in each `-I` directory, then in
`PARASOL_PATH`** — the order `@include` uses over in Solveig.
