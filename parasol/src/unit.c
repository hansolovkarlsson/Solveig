...
void parasol_unit_add_defaults(ParasolUnit *unit, const char *argv0)
{
    ...
    /* `bin/parasol` -> `bin/../lib`, which is `lib` beside it: the checkout's
       dialects, found without anybody saying where. */
    const char *slash = argv0 == NULL ? NULL : strrchr(argv0, '/');
    if (slash != NULL) {
        size_t directory = (size_t)(slash - argv0) + 1;
        static const char *suffix = "../lib";
        char *shipped = parasol_alloc(directory + strlen(suffix) + 1);
        memcpy(shipped, argv0, directory);
        memcpy(shipped + directory, suffix, strlen(suffix) + 1);
        parasol_unit_add_directory(unit, shipped);
        free(shipped);
    }

#if defined(PARASOL_LIB_DIR)
    parasol_unit_add_directory(unit, PARASOL_LIB_DIR);
#endif
}
