...
#ifndef PARASOL_DIALECT_H
#define PARASOL_DIALECT_H

#include "parasol/source.h"
#include "parasol/tree.h"

typedef enum { PARASOL_ASSOC_LEFT, PARASOL_ASSOC_RIGHT } ParasolAssoc;
...
typedef struct {
    ...
    int precedence;
    ParasolAssoc assoc;
    ParasolSpan declared_at;    /* for "previously declared here"            */
} ParasolInfix;
...
typedef enum {
    PARASOL_HOLE_EXPRESSION,    /* anything at all, and the default          */
    PARASOL_HOLE_NAME,
    PARASOL_HOLE_LITERAL,
    PARASOL_HOLE_BLOCK,
    PARASOL_HOLE_PLACE          /* something that may be assigned to         */
} ParasolHoleKind;
...
typedef struct {
    ...
    ParasolHoleKind *kinds;     /* what each will accept; parallel to params */
    int param_count;
    ParasolPatternPart *parts;  /* NULL for a call-shaped form */
    int part_count;
    ParasolNode *template;      /* owned */
    ParasolSpan declared_at;
} ParasolMacro;
...
/* The form a PARASOL_NODE_MACRO's `form` index names. */
