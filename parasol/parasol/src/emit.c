 ...
 * against the `.psol` offset that caused it, so the day something downstream --
...
static void put(ParasolEmitter *emitter, const char *chars, size_t length)
{
    if (emitter->length + length + 1 > emitter->capacity) {
        while (emitter->length + length + 1 > emitter->capacity)
            emitter->capacity = emitter->capacity < 256 ? 256
                                                        : emitter->capacity * 2;
        emitter->text = parasol_realloc(emitter->text, emitter->capacity);
    }
...
static void write(ParasolEmitter *emitter, const char *chars)
{
    put(emitter, chars, strlen(chars));
}

static void newline(ParasolEmitter *emitter)
{
    write(emitter, "\n");
    for (int i = 0; i < emitter->indent; i++) write(emitter, "    ");
}

static void record(ParasolEmitter *emitter, const ParasolNode *node)
{
    ...
    if (emitter->mapping_count == emitter->mapping_capacity) {
        ...
        emitter->mappings = parasol_realloc(emitter->mappings,
...
static void emit_node(ParasolEmitter *emitter, const ParasolNode *node);
...
static size_t flat_width(const ParasolNode *node)
{
    ParasolEmitter scratch;
    parasol_emitter_init(&scratch);
    scratch.flat = true;
    emit_node(&scratch, node);
    size_t width = scratch.length;
    parasol_emitter_free(&scratch);
    return width;
}

/* Where a generated line is asked to stop. Not a rule about style: a `.sol`
   Parasol wrote is read when something has gone wrong, and the reader is
   holding a column number out of a diagnostic. */
#define PARASOL_WIDTH 88

static void emit_body(ParasolEmitter *emitter, const ParasolNode *node,
...
static void emit_block(ParasolEmitter *emitter, const ParasolNode *node)
{
    ...
    bool multiline = !emitter->flat &&
                     (node->count > 1 ||
                      emitter->column + (int)flat_width(node) > PARASOL_WIDTH);
...
static void emit_node(ParasolEmitter *emitter, const ParasolNode *node)
{
    ...
    switch (node->kind) {
        case PARASOL_NODE_INTEGER:
            write(emitter, node->text);     /* the tag is part of the text */
            break;

        case PARASOL_NODE_FLOAT:
        case PARASOL_NODE_NAME:
            write(emitter, node->text);
            break;

        case PARASOL_NODE_STRING:
            write(emitter, "\"");
            write(emitter, node->text);
            write(emitter, "\"");
            break;

        case PARASOL_NODE_SYMBOL:
            write(emitter, "'");
            write(emitter, node->text);
            break;

        case PARASOL_NODE_INCLUDE:
            write(emitter, "@include \"");
            write(emitter, node->text);
            write(emitter, "\"");
            break;

        case PARASOL_NODE_SEND:
            emit_node(emitter, node->children[0]);
            write(emitter, ":");
            write(emitter, node->text);
            if (node->count > 1) {
        ...
        case PARASOL_NODE_ASSIGN:
            emit_node(emitter, node->children[0]);
            write(emitter, " := ");
            emit_node(emitter, node->children[1]);
            break;

        case PARASOL_NODE_ARRAY:
            write(emitter, "[");
        ...
        case PARASOL_NODE_BLOCK:
            emit_block(emitter, node);
            break;

        case PARASOL_NODE_MACRO:
        ...
        case PARASOL_NODE_SEQUENCE:
            /* A group. The module's own sequence never reaches here -- parasol_emit
...
{
    write(emitter,
          "; Generated by Parasol " PARASOL_VERSION ". Edit the .psol, not this.\n\n");
                                  ...
                                  int line, int column)
{
    ...
    for (int i = emitter->mapping_count - 1; i >= 0; i--) {
        const ParasolMapping *mapping = &emitter->mappings[i];
                        ...
                        const ParasolSource *source, const char *output_path)
{
    ...
    fprintf(file, "# parasol source map 1\n");
    fprintf(file, "# from %s\n", source->path);
    fprintf(file, "# to   %s\n", output_path);
    fprintf(file, "#\n");
    ...
    for (int i = 0; i < emitter->mapping_count; i++) {
        const ParasolMapping *mapping = &emitter->mappings[i];
        int line, column;
        parasol_span_position(mapping->span, &line, &column);
