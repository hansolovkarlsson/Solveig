...
char *parasol_path_identity(const char *path)
{
    ...
    if (resolved == NULL) return parasol_strndup(path, strlen(path));

    /* Onto this allocator, so every string a ParasolSource owns is freed the one
       way. `realpath` allocates with malloc and a caller cannot tell. */
    char *identity = parasol_strndup(resolved, strlen(resolved));
    free(resolved);
    return identity;
}
...
{
    source->path = parasol_strndup(path, strlen(path));
    source->identity = parasol_path_identity(path);
    source->text = NULL;
    source->length = 0;
    ...
    char *text = parasol_alloc((size_t)size + 1);
...
void parasol_source_adopt(ParasolSource *source, const char *path, const char *text)
{
    ...
    source->identity = parasol_strndup(path, strlen(path));
    source->length = strlen(text);
    source->text = parasol_strndup(text, source->length);
}
...
void parasol_span_position(ParasolSpan span, int *line, int *column)
{
    ...
    parasol_source_position(span.source, span.offset, line, column);
}
