...
static void name_set_add(NameSet *set, const char *name, size_t length)
{
    if (set->count == set->capacity) {
        ...
        set->names = parasol_realloc(set->names,
                                 (size_t)set->capacity * sizeof *set->names);
    }
    set->names[set->count++] = parasol_strndup(name, length);
}

static void name_set_collect(NameSet *set, const ParasolSource *source)
{
    ParasolLexer lexer;
    parasol_lexer_init(&lexer, source);
    for (;;) {
        ParasolToken token = parasol_lexer_next(&lexer);
        if (token.type == PARASOL_TOK_EOF) break;
        if (token.type != PARASOL_TOK_NAME) continue;
        char *name = parasol_strndup(token.start, (size_t)token.length);
...
typedef struct {
    const ParasolUnit *unit;
    const ParasolDialect *dialect;
...
void parasol_provenance_init(ParasolProvenance *provenance)
{
    provenance->nodes = NULL;
    provenance->count = provenance->capacity = 0;
}

void parasol_provenance_free(ParasolProvenance *provenance)
{
    for (int i = 0; i < provenance->count; i++)
        parasol_node_free(provenance->nodes[i]);
    free(provenance->nodes);
    parasol_provenance_init(provenance);
}

static void retain(ParasolProvenance *provenance, ParasolNode *node)
{
    if (provenance->count == provenance->capacity) {
        provenance->capacity = provenance->capacity < 16
                             ? 16 : provenance->capacity * 2;
        provenance->nodes = parasol_realloc(provenance->nodes,
...
static char *fresh_name(Expander *expander, const char *base)
{
    ...
    for (int n = 1; ; n++) {
        ...
        return parasol_strndup(buffer, strlen(buffer));
    }
}
...
typedef struct {
    const ParasolMacro *macro;
    const ParasolNode *use;         /* the arguments live here */
...
static void collect_binders(const ParasolNode *node, NameSet *binders)
{
...
static int parameter_index(const ParasolMacro *macro, const char *name)
{
...
static ParasolNode *instantiate(const Instance *instance, const ParasolNode *template)
{
    if (template->kind == PARASOL_NODE_NAME) {
        int index = parameter_index(instance->macro, template->text);
        if (index >= 0) return parasol_node_copy(instance->use->children[index]);
    }

    ParasolNode *copy = parasol_node_new(template->kind, template->span);
    ...
    if (template->text != NULL) {
        const char *text = template->kind == PARASOL_NODE_NAME
                         ? renamed(instance, template->text)
                         : template->text;
        copy->text = parasol_strndup(text, strlen(text));
    }

    for (int i = 0; i < template->param_count; i++) {
        const char *name = renamed(instance, template->params[i]);
        parasol_node_add_param(copy, name, (int)strlen(name));
    }
    for (int i = 0; i < template->temp_count; i++) {
        const char *name = renamed(instance, template->temps[i]);
        parasol_node_add_temp(copy, name, (int)strlen(name));
    }
    for (int i = 0; i < template->count; i++)
        parasol_node_add(copy, instantiate(instance, template->children[i]));

    return copy;
}

static ParasolNode *expand_node(Expander *expander, ParasolNode *node, int depth);
static void note_trail(Expander *expander, const ParasolNode *node);
 ...
 * was found in. */
static bool satisfies(const ParasolNode *node, ParasolHoleKind kind)
{
    switch (kind) {
        case PARASOL_HOLE_EXPRESSION: return true;
        case PARASOL_HOLE_NAME:       return node->kind == PARASOL_NODE_NAME;
        case PARASOL_HOLE_BLOCK:      return node->kind == PARASOL_NODE_BLOCK;
        case PARASOL_HOLE_PLACE:      return assignable(node);
        case PARASOL_HOLE_LITERAL:
            return node->kind == PARASOL_NODE_INTEGER ||
                   node->kind == PARASOL_NODE_FLOAT ||
                   node->kind == PARASOL_NODE_STRING ||
                   node->kind == PARASOL_NODE_SYMBOL;
    }
    return true;
}
...
static void check_arguments(Expander *expander, const ParasolNode *use,
                            const ParasolMacro *macro, const ParasolSpan *written)
{
    for (int i = 0; i < macro->param_count && i < use->count; i++) {
        ParasolHoleKind kind = macro->kinds[i];
        const ParasolNode *argument = use->children[i];
        if (satisfies(argument, kind)) continue;

        const char *wanted = parasol_hole_kind_name(kind);
        const char *got = parasol_node_kind_name(argument->kind);

        parasol_error(expander->diag, written[i],
         ...
         * neither reached it: `lib/clike.psol` spends eleven lines at its own
        ...
        if (kind == PARASOL_HOLE_BLOCK)
            parasol_note(expander->diag, written[i],
                     "wrap it in braces -- '{' before this and '}' after it");
        parasol_note(expander->diag, macro->declared_at,
...
#define PARASOL_EXPANSION_LIMIT 256

static ParasolNode *expand_macro(Expander *expander, ParasolNode *use, int depth,
                               const ParasolSpan *written)
{
    ...
    const ParasolMacro *macro = parasol_dialect_form_at(expander->dialect, use->form);
    if (macro == NULL) {
        ...
        parasol_error(expander->diag, use->span,
    ...
    instance.to = binders.count > 0
                ? parasol_alloc((size_t)binders.count * sizeof *instance.to) : NULL;
    for (int i = 0; i < binders.count; i++)
        instance.to[i] = fresh_name(expander, binders.names[i]);

    ParasolNode *result = instantiate(&instance, macro->template);
...
static ParasolNode *expand_node(Expander *expander, ParasolNode *node, int depth)
{
    if (depth > PARASOL_EXPANSION_LIMIT) {
        parasol_error(expander->diag, node->span,
    ...
    ParasolSpan *written = NULL;
    if (node->kind == PARASOL_NODE_MACRO && node->count > 0) {
        written = parasol_alloc((size_t)node->count * sizeof *written);
        for (int i = 0; i < node->count; i++)
            written[i] = parasol_node_extent(node->children[i]);
    }
    ...
    if (node->kind == PARASOL_NODE_MACRO) {
        ParasolNode *expanded = expand_macro(expander, node, depth, written);
        free(written);
        return expanded;
    }
...
typedef struct Frame {
    struct Frame *parent;
    ParasolNode *block;             /* NULL at the module's own level */
} Frame;

static bool binds(const ParasolNode *block, const char *name)
{
...
static void add_capture(Expander *expander, ParasolNode *block, const char *name)
{
    ...
    if (expander->capture_count == expander->capture_capacity) {
        ...
        expander->captures = parasol_realloc(expander->captures,
...
static void find_captures(Expander *expander, ParasolNode *node, Frame *frame)
{
    if (node->kind == PARASOL_NODE_NAME && node->scope != 0) {
    ...
    /* A group borrows the frame it sits in rather than making one, so only a
       ...
       frame if Parasol read them, and it does not yet. */
    Frame inner = { frame, node };
    Frame *next = node->kind == PARASOL_NODE_BLOCK ? &inner : frame;
    for (int i = 0; i < node->count; i++)
 ...
 * renamed frame. */
static void rename_in(ParasolNode *node, const char *from, const char *to,
                      uint32_t scope)
{
    if (node->kind == PARASOL_NODE_BLOCK && node->scope == scope &&
        binds(node, from))
        return;                                     /* shadowed from here down */

    if (node->kind == PARASOL_NODE_NAME && node->scope == scope &&
        strcmp(node->text, from) == 0) {
        free(node->text);
        node->text = parasol_strndup(to, strlen(to));
    }
...
static void rename_binder(ParasolNode *block, const char *from, const char *to)
{
    for (int i = 0; i < block->param_count; i++)
        if (strcmp(block->params[i], from) == 0) {
            free(block->params[i]);
            block->params[i] = parasol_strndup(to, strlen(to));
        }
    for (int i = 0; i < block->temp_count; i++)
        if (strcmp(block->temps[i], from) == 0) {
            free(block->temps[i]);
            block->temps[i] = parasol_strndup(to, strlen(to));
        }
}

static void protect_free_references(Expander *expander, ParasolNode *module)
{
    ...
    for (int i = 0; i < expander->capture_count; i++) {
        ...
        char *from = parasol_strndup(capture->name, strlen(capture->name));
...
static void note_trail(Expander *expander, const ParasolNode *node)
{
    for (const ParasolNode *use = node->introduced_by; use != NULL;
        ...
        parasol_note(expander->diag, use->span,
                 "in the expansion of '%s', written here", use->text);

        const ParasolMacro *macro = parasol_dialect_form_at(expander->dialect,
                                                    use->form);
        if (macro != NULL && macro->declared_at.source != use->span.source)
            parasol_note_from(expander->diag, macro->declared_at, "declared in");
    }
}
...
static void validate(Expander *expander, const ParasolNode *node)
{
    if (node->kind == PARASOL_NODE_ASSIGN && !assignable(node->children[0])) {
        ...
        parasol_error(expander->diag, parasol_node_extent(node->children[0]),
...
bool parasol_expand(ParasolNode *module, const ParasolUnit *unit,
                const ParasolDialect *dialect, ParasolDiagnostics *diag,
                ParasolProvenance *provenance)
{
    Expander expander;
    expander.unit = unit;
