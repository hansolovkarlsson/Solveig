...
static void test_counted_loops_count(void)
{
    ...
    assert(run(&vm, &chunk,
        ...
        "up := array:new. [#1,#3]:loop({ n | up:add(n) })."
        "stepped := array:new. [#1,#10,#3]:loop({ n | stepped:add(n) })."
        "down := array:new. [#3,#1,#0:sub(#1)]:loop({ n | down:add(n) })."
    ...
    sol_chunk_free(&chunk); sol_vm_free(&vm);
    printf("  repeat and loop count what they say\n");
}
...
static void test_counted_loops_over_nothing(void)
{
    ...
    assert(run(&vm, &chunk,
        ...
        "d := #0. [#5,#1]:loop({ n | d := d:add(#1) })."
        "e := #0. [#1,#5,#0:sub(#1)]:loop({ n | e := e:add(#1) })."
...
static void test_counted_loops_refuse(void)
{
    static const char *refused[] = {
        ...
        "#3:repeat.",
        "{ #1 }:repeat(1.5).",
        "[#1,#5,#0]:loop({ n | n }).",     /* a step of #0 never finishes */
        "[#1,1.5,#1]:loop({ n | n }).",
        "[#1,#3]:loop({ n | n }, #4).",
        "[#1,#3]:loop({ }).",                 /* the block is handed the index */
    };
...
static void test_an_error_in_the_body_stops_the_loop(void)
{
    ...
    assert(run(&vm, &chunk,
        "seen := #0."
        "[#1,#10]:loop({ n | seen := seen:add(#1). nil:frobnicate }).") == SOL_RUNTIME_ERROR);
    assert(SOL_AS_INT(global(&vm, "seen")) == 1);
    sol_chunk_free(&chunk); sol_vm_free(&vm);
...
static void test_a_huge_step_terminates(void)
{
    ...
    assert(run(&vm, &chunk,
        "seen := #0."
        "[#9223372036854775806,#9223372036854775807,#1]:loop("
    ...
    assert(run(&vm, &chunk,
        "seen := #0."
        "[#9223372036854775806,#9223372036854775807,#9223372036854775807]:loop("
        "    { n | seen := seen:add(#1) }).") == SOL_OK);
    assert(SOL_AS_INT(global(&vm, "seen")) == 1);
    sol_chunk_free(&chunk); sol_vm_free(&vm);
    ...
    assert(run(&vm, &chunk,
        "seen := #0."
        "[#0:sub(#9223372036854775807),#0:sub(#9223372036854775807),#0:sub(#1)]:loop("
        "    { n | seen := seen:add(#1) }).") == SOL_OK);
    assert(SOL_AS_INT(global(&vm, "seen")) == 1);
    sol_chunk_free(&chunk); sol_vm_free(&vm);
