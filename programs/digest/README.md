...
**Not a Parasol feature.** A third program, written for the reason the first two
...
operator half of Parasol has never had a customer**: `lib/arith.psol` declares
operators for arithmetic that Solveig can already write as sends, and
`examples/utf8.psol` is twenty lines.
...
Parasol's ROADMAP has claimed the answer to that in the abstract since 0.1.0 —
...
It also lands on a second thing Solveig measured and Parasol has not:
...
A Parasol template is expanded, not called. So the readable spelling and the
...
| **3. A template that names a hole twice will evaluate it twice.** | `rotr` is `(x >> n) \| (x << (32-n))` and `left` appears on both sides of the bar. Parasol has hygiene but no way to say *bind this once*. Predicted to bite, predicted to have no clean fix today, and predicted to be the sharpest finding **against** Parasol. Mitigation predicted: rotations apply to plain variables, so the duplication re-reads rather than re-computes. |
| **4. Nothing folds the constants a template introduces.** | `x >>> #2` expands with `#32:sub(#2)` inside it, and that subtraction is a send at run time, once per rotation, ten times a round. Predicted: real, measurable, and correctly Parasol's problem rather than Solveig's — an expander that does not evaluate is the whole reason `.psol` files can be read without running them. |
...
because `\/` was doing double duty -- logical or in `lib/arith.psol`, bitwise or
...
| **2. The dialect carries a rule** | **Right, and it is the finding of the program.** Solveig's version writes the mod-2³² discipline by hand; this one does not, because `+` *is* addition modulo 2³². A dialect here is not shorter code, it is code that cannot express the mistake. *Counted properly on 2026-09-02 for [does-it-pay.md](../../docs/PARASOL-DOES-IT-PAY.md): Solveig's file has 24 `bitAnd`s in code, of which **18 are `bitAnd(mask)`** and six are real algorithm ANDs; this file has **0** masks and **one** `bitAnd`, which is a byte extract rather than a mask. The figures said "twenty-three" and "none" — near enough to be repeated three times and wrong in both directions.* |
...
**The template gives back 98% of what it saves.** Parasol avoids the frame and
...
version of the question [rules-and-logic.md](../../docs/PARASOL-RULES-AND-LOGIC.md) asks
...
bounds four calls deep in generated code. `lib/arith.psol` puts `*` at 70 for
...
compose.** `@use "sha2.psol"` beside `lib/control.psol` collides, because
control.psol uses arith.psol. Parasol names both declarations and the whole `@use`
...
was on `+`, `-`, `~` and `\/`, and that Parasol **reports all four**. Those four
are the ones whose *meaning* differs; Parasol reports every redeclaration, which
...
**A dialect file must not declare `@language`.** Putting one in `sha2.psol` gives
