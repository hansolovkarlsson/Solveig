...
#
#     python3 programs/sql/generate.py DIR SEED COUNT
#     python3 programs/sql/generate.py DIR SEED COUNT writable
#
...
#
# `writable` keeps the build inside what lib/sqlite.sol's writer does: no row or
...
def case(seed, number, out, writable=False):
    ...
    lines = []
    queries = []
    lines.append('-- generated by programs/sql/generate.py, seed %d case %d' % (seed, number))
    if page != 4096:
