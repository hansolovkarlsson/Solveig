...
| **4. `<=` and `>=` get their second customer, and it is the interpreter** | **Right, in both halves.** Five sends of `lessOrEqual` and `greaterOrEqual`, every one of them the interpreter's own bounds test — the `FOR` limit and `pc <= program:size`. BASIC's own `<=` is a two-character token `twoCharOp` reads at run time and is never an operator this module declares. [ROADMAP.md](../../docs/ROADMAP.md)'s entry has its second customer and is **still open**: the count is two for `<=` and one each for `>=` and `!=`, so the bar is met for one of the three, and splitting the family is worse than either answer. The first time *one customer is not enough* has been ambiguous rather than unmet. |
...
| **6. Proto helps with less of this than it looks** | **Right.** Sixteen lines of dialect against 252 of program. The forms cover the lexer's character tests, the parser's cursor and the three moves on the counter. They touch **none** of the environment, the return stack or the value model, which are the three things that make this an interpreter. |
...
Every other increment in a 252-line interpreter is inside a template.
