...
| **Alternation** | `expr = a \| b`, and a hole that accepts one of several kinds. `lib/clike.psol` wanted the second and worked around it. |
...
Two grammars run: `examples/calc.psol` evaluates `100 / 5 - 3 * 4` with
precedence, and `examples/sexpr.psol` parses `(a (b c) d)` into nested arrays.
...
The first draft of `peg.psol` declared its cursor forms as patterns, and both
...
That was written up here as a limitation of Parasol — *a form's trailing hole
...
author's examples.** Only `<b: block>` survived in `peg.psol`, because the table
