...
**[ROADMAP.md](../../docs/parasol/ROADMAP.md) wants a second program with a
...
| | |
| --- | --- |
| **1. Naming the scale produces an unfolded constant.** | A fixed-point dialect wants its precision written once, and `@syntax scale => #100.` is how Parasol spells that. Round-half-up then wants half a unit, which is `scale:div(#2)` — and expands to `#100:div(#2)`, a send on two literals evaluated once per multiply. Predicted: **digest's `#32:sub(#17)` in a second domain, arrived at from a different direction**, and exactly the second customer the roadmap asks for. |
...
number. Which is where [ROADMAP.md](../../docs/parasol/ROADMAP.md) had already put it,
...
**`#-1225` is not a literal in Parasol, and is one in Solveig.** Solveig's grammar
says so plainly:
...
decides. Nothing in the repository had done it; `lib/arith.psol` uses `!` for the
...
fourth program. The collision rules could not come up — `money.psol` is
standalone for digest's reason, which this program confirms rather than extends:
`@use "arith.psol"` after it silently buys integer `*` in place of fixed-point
