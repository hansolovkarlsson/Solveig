...
Six versions of Parasol were built with four toy examples and no customer.
...
| **5. Solveig will bite before Parasol does.** | Most likely [3.1](https://hansolovkarlsson.github.io/Solveig/docs/ROADMAP.html) — a block outliving the frame it was written in — if the codegen is built the combinator way. `lib/scan.sol` already hit it and says so. |
...
| **2. The group parens will grate** | **Right, and in a place not predicted.** They are on every multi-statement branch in the compiler, as expected — but they are also in `asm.psol`, because a *template* is one expression too. `load <d> adr <s>` emits two instructions and needs a group to do it. The wart is not the pattern language's; it is that a template is an expression. |
...
| **4. The dialect will mostly be `concat`** | **Right, and it sharpened.** Every form in `asm.psol` is one `fill`. The ones that earned their keep hide *the operand order as well as the joining* — `store "x0" slot n` against `emit("    str {}, [x29, #-{}]":fill([r, n:mul(#8)]))`, where the slot arithmetic is the part you would get wrong. |
| **5. Solveig will bite first** | **Wrong.** No 3.1, no block outliving its frame, nothing. A lexer, a recursive-descent parser and a code generator went in without Solveig complaining once, with `lib/scan.sol` doing the cursor work. The things that bit were Parasol's and the author's. |
...
**Fixed in Parasol 0.7.0, and this program is why.** The obvious answer -- also
...
may now stand for a *template*, `lib/arith.psol` declares them, and every one of
the six expressions above is now written with them.

*Spelled `/\` and `\/` when this was written; `&&` and `||` since 0.10.0.*
...
Ember has `end` on its blocks and Parasol's own forms do not, which is the
