/* proto -- the Proto compiler. A .pro in, Solveig source out.
 *
...
{
    printf("%s " PROTO_VERSION " (emits Solveig source; needs solas "
           PROTO_SOLVEIG_MINIMUM " or later to compile it)\n", NAME);
}

static void usage(FILE *out)
{
    fprintf(out,
        "usage: proto [options] <file.pro>\n"
        "\n"
        "Compiles a Proto module to Solveig source.\n"
        "\n"
        ...
        "directory in order, then in PROTO_PATH (colon-separated).\n"
        "\n"
        "The generated file is an artefact. Compile it with solas and keep the\n"
        "map: it is what turns a position in the .sol back into the .pro line\n"
        "somebody actually wrote.\n");
}

/* "prog.pro" -> "prog.sol"; anything else just gains ".sol". */
static char *default_output_path(const char *source_path)
{
    size_t length = strlen(source_path);
    bool has_pro = length > 4 && strcmp(source_path + length - 4, ".pro") == 0;
    size_t base = has_pro ? length - 4 : length;

    char *out = proto_alloc(base + 5);
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], "-I") == 0) {
            ...
            proto_unit_add_directory(&unit, argv[i]);
            continue;
        }
    ...
    proto_unit_add_environment(&unit);
    ...
    if (want_tree) {
        proto_node_dump(module, stdout, 0);
        proto_node_free(module);
        proto_provenance_free(&provenance);
        proto_dialect_free(&dialect);
        proto_unit_free(&unit);
    ...
    int status = 0;
    if (!proto_emit_write(&emitter, destination)) {
        fprintf(stderr, NAME ": cannot write %s\n", destination);
        status = 74;
    } else if (want_map) {
        char *map = map_path_for(destination);
        if (!proto_emit_map_write(&emitter, map, source, destination)) {
            fprintf(stderr, NAME ": cannot write %s\n", map);
            status = 74;
        }
        free(map);
