...
nothing, while every `.pro` writes it as a ritual — in the directive the README
...
Proto still isn't linked from Solveig — the hold expired 2026-08-31, so that's
now just your call.
...
> So exactly how does it work? Proto runs the rules on the code and replaces the
> parts that matches?
...
`proto/cmd/main.c:123-173`, in order:

1. **Read the header.** Every `@infix`/`@prefix`/`@syntax`/`@use` at the top of
   the file builds a `ProtoDialect` — a precedence table and a form table
   (`proto/src/dialect.c`).
2. **Parse the body under that dialect.** The parser consults the table as it
   goes. `proto/src/reader.c` has holes at exactly three places (`infix`,
   `unary`, `form` in `docs/GRAMMAR.md:22-38`); everything else in the grammar is
   fixed for every `.pro` there will ever be.
3. **Expand.** The tree now has a `PROTO_NODE_MACRO` at every use of a declared
   form. `proto_expand` (`proto/src/expand.c`) replaces each one with an instance
...
Three things at once (`proto/include/proto/expand.h`), because doing any later
...
You can see all three in `examples/forms.pro` → `examples/forms.sol`:
...
**Parsing never runs anything.** A tool can read any `.pro` top to bottom and
...
Not because of anything the program is about. Proto's fixed token set and core
...
fixed by the lexer, which no `.pro` can change.

### Reading 1: the body is a different language

This is `programs/ember` — a lexer, parser and **ARM64 code generator** written
in Proto. It emits machine instructions, and that never touched Proto's token set
...
no vote on which characters Proto's lexer reserves. So a Pascal compiler written
in Proto would still not be able to declare `|`.
...
the repetition it wanted was in the object language, where Proto's syntax doesn't
reach.

### Reading 2: Proto emits Pascal instead of Solveig
...
`proto/src/emit.c` is the only file that has ever heard of Solveig. Replacing it
...
operator character a Proto dialect can claim today. `..` would be worse than a
...
> Explain to me the process and steps how Proto works. I'm still confounded by
...
Proto treats it as a *statement* and passes it through.)

### Step 1 — gather the files

`proto_unit_read` loads `trace.pro`. The `@use "control.pro"` pulls in
`lib/control.pro`, which itself does `@use "arith.pro"`. Each file is read
**once** — a diamond costs nothing — and they all live in one `ProtoUnit` with a
...
Reading top to bottom, the directives fill a `ProtoDialect`:
...
`x:msg(y)`, `{ }`, `.` between statements — is fixed for every `.pro` there will
ever be.
...
`proto_expand` replaces each `MACRO` with its template. Run `--tree` on that file
and you can see all three things it does at once:

```
send whileTrue @1178+9 scope:1 from:while      <- template's span, in control.pro
  block @1172+1 scope:1 from:while
    send greaterThan @70+1                     <- argument's own span, in trace.pro
      name n @68+1
```

- **substitution** — `{ t }` became a block wrapping `n:greaterThan(#0)`, and
  that subtree kept offset `@70`, its position *in trace.pro*. An error inside an
...
Only `proto/src/emit.c` has ever heard of Solveig. It walks the expanded tree and
prints:
...
```
5:1   24:40  1172  lib/control.pro     <- the whileTrue came from the template
...
Then Solveig takes over, and Proto is done:

```sh
proto trace.pro -o trace.sol --map
...
can tokenise and parse any `.pro` top to bottom. This is what the refused
...
> So is it fair to say that in a way Proto is a very flexible and smart
> precompiler?

**"Smart precompiler" gets the position right and the mechanism wrong.** Proto
...
won't parse. Proto's matching happens *inside* the parser, and expansion operates
...
syntax macros. The line Proto keeps that Lisp does not is that **nothing runs at
...
It is also worth saying that Proto is not "very flexible" in the unbounded sense,
